...
for istartdist in range(0,len(lensToStart)):
 ...
 enddist = lensToEnd[istartdist]
 unpicklefile = open('synlocs'+str(startdist)+'-'+str(enddist)+'.sav', 'r')
 unpickledlist = pickle.load(unpicklefile)
 unpicklefile.close()
 ...
 for icell in range(0,7):
  ...
  if len(sys.argv) > 2 and int(sys.argv[2]) != icell:
    continue
  ...
  for igene in range(0,len(MT)):
    ...
    for imut in range(0,len(MT[igene])):
      ...
      for iallmutval in range(0,cumprodnVals[len(MT[igene][imut])-1]):
        ...
        maxCac = 0
        maxCadc = 0
        if exists('thresholddistalamp'+str(startdist)+'-'+str(enddist)+'_cs'+str(icell)+'_'+str(counter)+'.sav'):
          print 'thresholddistalamp'+str(startdist)+'-'+str(enddist)+'_cs'+str(icell)+'_'+str(counter)+'.sav exists, continuing'
          continue
...
""")
        ...
        file = open('thresholddistalamp'+str(startdist)+'-'+str(enddist)+'_cs'+str(icell)+'_'+str(counter)+'.sav', 'w')
  ...
  #file = open('thresholddistalamp'+str(startdist)+'-'+str(enddist)+'_'+str(counter)+'.sav', 'w')
  #pickle.dump(picklelist,file)
  #file.close()
